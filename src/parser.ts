...
import { isAlpha, isAlphaNum, isDigit, isWhitespace } from './util.js';
import type {
  AstNode,
  BinNode,
  CallNode,
  ...
  while (i < src.length) {
    ...
    if (c === '$') {
      ...
      if (i >= src.length || !isAlpha(src[i])) {
        throw new TokenizeError(start, 'expected identifier after $');
      }
      let name = '';
    ...
    if (isAlpha(c)) {
      ...
      if (name === 'true') {
        tokens.push({ kind: 'bool', value: true, start, end: i });
        continue;
      }
      if (name === 'false') {
        tokens.push({ kind: 'bool', value: false, start, end: i });
        continue;
      }
      if (name === 'null') {
        tokens.push({ kind: 'null', value: null, start, end: i });
        continue;
      }
      const upper = name.toUpperCase();
      if (upper === 'AND' || upper === 'OR' || upper === 'NOT') {
        tokens.push({ kind: 'logic', value: upper, start, end: i });
        continue;
      }
  ...
  expect(kind: TokenKind): Token {
    ...
    if (t.kind !== kind) {
      throw new ConfigError(
        'S5_EXPR_PARSE',
        `expected ${kind}, got ${String(t.value)} (${t.kind}) at col ${t.start}`,
      );
    }
    return t;
  }
...
export function parseExpr(src: string): AstNode {
  ...
  if (st.peek().kind !== 'eof') {
    ...
    throw new ConfigError(
      'S5_EXPR_PARSE',
      `unexpected token '${String(t.value)}' at col ${t.start}`,
    );
  }
  return node;
}
...
function parsePrefix(st: ParseState): AstNode {
  ...
  if (t.kind === 'num') {
    return { k: 'lit', type: 'num', value: t.value, loc: { start: t.start, end: t.end } } as LitNode;
  }
  if (t.kind === 'str') {
    return { k: 'lit', type: 'str', value: t.value, loc: { start: t.start, end: t.end } } as LitNode;
  }
  if (t.kind === 'bool') {
    return { k: 'lit', type: 'bool', value: t.value, loc: { start: t.start, end: t.end } } as LitNode;
  }
  if (t.kind === 'null') {
    return { k: 'lit', type: 'null', value: null, loc: { start: t.start, end: t.end } } as LitNode;
  }
  if (t.kind === 'var') {
    return { k: 'var', name: String(t.value), loc: { start: t.start, end: t.end } } as VarNode;
  }
  if (t.kind === 'lparen') {
    const inner = parseExpression(st, 0);
    st.expect('rparen');
    return inner;
  }
  if (t.kind === 'op' && (t.value === '-' || t.value === '!')) {
    ...
    return {
      ...
      op: t.value as '-' | '!',
      operand,
      loc: { start: t.start, end: operand.loc.end },
    } as UnNode;
  }
  ...
  if (t.kind === 'ident') {
    if (st.peek().kind !== 'lparen') {
      throw new ConfigError(
        'S5_EXPR_PARSE',
        `bare identifier '${String(t.value)}' not allowed (use $var) at col ${t.start}`,
      );
    }
    st.next();
    const args: AstNode[] = [];
...
function parseInfix(st: ParseState, left: AstNode, opTok: Token, lbp: number): AstNode {
  ...
  const rhsPrec = isRightAssoc ? lbp - 1 : lbp;
  const right = parseExpression(st, rhsPrec);
...
export function walkAst(node: AstNode, visit: (n: AstNode) => void): void {
  ...
  } else if (node.k === 'logic') {
    for (const child of node.operands) walkAst(child, visit);
  } else if (node.k === 'call') {
...
export function collectVarRefs(node: AstNode): Set<string> {
  ...
  walkAst(node, (n) => {
    if (n.k === 'var') refs.add(n.name);
  });
  return refs;
}

export function collectFuncCalls(node: AstNode): Set<string> {
  const calls = new Set<string>();
  walkAst(node, (n) => {
    if (n.k === 'call') calls.add(n.name);
  });
  return calls;
}

export function parseCellPattern(raw: unknown): CellPattern {
  if (typeof raw !== 'string') return { k: 'literal', value: raw };
  if (raw === '*') return { k: 'wildcard' };

  const rangePos = findRangeOperator(raw);
  if (rangePos >= 0) {
    const lo = parseSignedNumber(raw.slice(0, rangePos));
    const hi = parseSignedNumber(raw.slice(rangePos + 2));
    if (lo !== null && hi !== null) return { k: 'range', lo, hi };
  }
  ...
  try {
    ...
    if (e instanceof TokenizeError) return { k: 'literal', value: raw };
    throw e;
  }
  const main = toks.slice(0, -1);

  if (main.length === 2 && main[0].kind === 'op' && main[1].kind === 'num') {
    const op = main[0].value;
    if (op === '>' || op === '<' || op === '>=' || op === '<=' || op === '!=') {
      return { k: 'cmp', op, value: main[1].value as number };
    }
  }
  if (
    main.length === 3 &&
    ...
    const op = main[0].value;
    if (op === '>' || op === '<' || op === '>=' || op === '<=' || op === '!=') {
      return { k: 'cmp', op, value: -(main[2].value as number) };
    }
  }
  if (main.length === 1 && main[0].kind === 'num') {
    return { k: 'literal', value: main[0].value };
  }
  if (main.length === 2 && main[0].kind === 'op' && main[0].value === '-' && main[1].kind === 'num') {
    return { k: 'literal', value: -(main[1].value as number) };
  }
  if (main.length === 1 && (main[0].kind === 'bool' || main[0].kind === 'null')) {
    return { k: 'literal', value: main[0].value };
  }
  if (main.length === 1 && main[0].kind === 'str') {
    return { k: 'literal', value: main[0].value };
  }
  return { k: 'literal', value: raw };
}

function findRangeOperator(s: string): number {
